...
def slot_date(day_label: str) -> date:
    """Return the demo-week date for a Russian weekday label."""
    target_code = CODE_BY_RU[day_label]
    monday = DEMO_TODAY - timedelta(days=DEMO_TODAY.weekday())
    for offset in range(7):
        candidate = monday + timedelta(days=offset)
        if WEEKDAY_BY_INDEX[candidate.weekday()] == target_code:
            return candidate
    return monday
...
def blocking_event_for_slot(employee: Dict[str, Any], events: List[Dict[str, Any]], day_label: str, hour: int) -> Optional[Dict[str, Any]]:
...
def calculate_employee_metrics(
    ...
    outside_work_events = sum(1 for event in current_events if is_event_outside_work_time(event, employee))
...
def build_frontend_employee_list(
    employees: List[Dict[str, Any]],
    events: List[Dict[str, Any]],
    hr_profiles: List[Dict[str, Any]],
    absences: Optional[List[Dict[str, Any]]] = None,
) -> List[Dict[str, Any]]:
    return [to_frontend_employee(item, item["metrics"]) for item in build_employee_list(employees, events, hr_profiles, absences)]
...
def build_conflicts(employees: List[Dict[str, Any]], events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def build_data_mismatches(employees: List[Dict[str, Any]], hr_profiles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    hr_by_employee_id = {int(profile["employee_id"]): profile for profile in hr_profiles}
...
def build_frontend_summary(frontend_employees: List[Dict[str, Any]], conflicts: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    return {
        ...
        "outdated": sum(1 for employee in frontend_employees if employee["metrics"]["days_since_update"] > MAX_DAYS_WITHOUT_UPDATE),
...
def build_recommendations(
    ...
    for employee in employees:
        metrics = calculate_employee_metrics(employee, events, hr_by_employee_id.get(int(employee["id"])), absences)
...
def build_groups(
    ...
    frontend_employees = build_frontend_employee_list(employees, events, hr_profiles, absences)

    return {
        "actual": [
            employee for employee in frontend_employees
            if employee["metrics"]["schedule_actuality"] >= 0.70 and employee["metrics"]["risk"] < 0.50
        ],
        "outdated": [employee for employee in frontend_employees if employee["metrics"]["days_since_update"] > MAX_DAYS_WITHOUT_UPDATE],
        "outsideWorkMeetings": [employee for employee in frontend_employees if employee["metrics"]["calendar_conflict_count"] > 0],
        "highLoad": [employee for employee in frontend_employees if employee["metrics"]["load"] >= 0.80],
        "timezoneConflict": [employee for employee in frontend_employees if employee["metrics"]["timezone_mismatch"]],
        "hrMismatch": [employee for employee in frontend_employees if employee["metrics"]["hr_mismatch"]],
        "needsConfirmation": [
            employee for employee in frontend_employees
            if employee["metrics"]["risk"] >= 0.50 or employee["metrics"]["days_since_update"] > 60
        ],
    }
...
def build_risk_explanation(
    ...
    absences = absences or []
    employee = next((item for item in employees if int(item["id"]) == int(employee_id)), None)
    if not employee:
        return None

    hr_profile = next((item for item in hr_profiles if int(item["employee_id"]) == int(employee_id)), None)
    metrics = calculate_employee_metrics(employee, events, hr_profile, absences)

    days_factor = clamp(metrics["days_since_update"] / MAX_DAYS_WITHOUT_UPDATE)
    factors = [
        {
            "factor": "days_since_update",
            "label": "Давность обновления",
            "value": metrics["days_since_update"],
            "score": round2(days_factor),
            "impact": impact(days_factor),
            "explanation": f"График обновлялся {metrics['days_since_update']} дней назад. Порог для MVP — {MAX_DAYS_WITHOUT_UPDATE} дней.",
        },
        {
            "factor": "outside_work_ratio",
            "label": "Встречи вне рабочего времени",
            "value": metrics["outside_work_ratio"],
            "score": metrics["outside_work_ratio"],
            "impact": impact(metrics["outside_work_ratio"]),
            "explanation": f"{metrics['outside_work_events']} из {metrics['total_events']} событий выходят за рабочий график.",
        },
        {
            "factor": "load",
            "label": "Загрузка",
            "value": metrics["load"],
            "score": metrics["load"],
            "impact": impact(metrics["load"]),
            "explanation": f"Занято {metrics['busy_hours']} ч из {metrics['work_hours']} рабочих часов за неделю.",
        },
        {
            "factor": "timezone_mismatch",
            "label": "Часовой пояс",
            "value": metrics["timezone_mismatch"],
            "score": float(metrics["timezone_mismatch"]),
            "impact": impact(float(metrics["timezone_mismatch"])),
            "explanation": "Часовой пояс отличается от HR-профиля." if metrics["timezone_mismatch"] else "Расхождения часового пояса нет.",
        },
        {
            "factor": "hr_mismatch",
            "label": "HR-данные",
            "value": metrics["hr_mismatch"],
            "score": float(metrics["hr_mismatch"]),
            "impact": impact(float(metrics["hr_mismatch"])),
            "explanation": "Формат или часы отличаются от HR-профиля." if metrics["hr_mismatch"] else "Расхождения с HR-профилем нет.",
        },
    ]
    ...
    return {
        ...
        "formula": "0.30*(1-A) + 0.25*C + 0.25*L + 0.10*timezone_mismatch + 0.10*hr_mismatch",
        "factors": factors,
        "recommendedActions": build_recommendation_texts(employee, metrics),
    }
    ...
    for employee in employees:
        ...
        if metrics["risk"] >= 0.50 or metrics["days_since_update"] > 60:
            notifications.append({
                "id": f"confirm-{employee['id']}",
                "recipientRole": "HR",
                "employeeId": int(employee["id"]),
                "employee": employee["name"],
                "title": "Запросить подтверждение рабочего графика",
                "reason": f"Риск {percent(metrics['risk'])}%, последнее обновление {metrics['days_since_update']} дней назад.",
                "priority": "Срочно" if metrics["risk"] >= 0.75 else "Важно",
                "action": "send_schedule_confirmation_request",
            })

        if metrics["load"] >= 0.80:
            notifications.append({
                "id": f"load-{employee['id']}",
                "recipientRole": "Руководитель",
                "employeeId": int(employee["id"]),
                "employee": employee["name"],
                "title": "Проверить перегруз сотрудника",
                "reason": f"Загрузка {percent(metrics['load'])}%, новые встречи лучше не назначать.",
                "priority": "Срочно",
                "action": "review_employee_load",
            })

        if metrics["timezone_mismatch"] or metrics["hr_mismatch"]:
            notifications.append({
                "id": f"data-{employee['id']}",
                "recipientRole": "HR",
                "employeeId": int(employee["id"]),
                "employee": employee["name"],
                "title": "Сверить данные HR и календаря",
                "reason": "Есть расхождение часового пояса, формата работы или рабочих часов.",
                "priority": "Важно",
                "action": "review_hr_profile",
            })

    for conflict in build_conflicts(employees, events)[:5]:
        notifications.append({
            "id": f"meeting-{conflict['id']}",
            "recipientRole": "PM",
            "employeeId": conflict["employeeId"],
            "employee": conflict["employee"],
            "title": "Перенести встречу вне рабочего времени",
            "reason": f"{conflict['title']}: {conflict['day']} {conflict['time']}. {conflict['reason']}",
            "priority": "Срочно" if conflict["severity"] == "Высокая" else "Важно",
            "action": "reschedule_meeting",
        })

    return sorted(notifications, key=lambda item: PRIORITY_ORDER.get(item["priority"], 9))
...
def build_worktime_overview(
    ...
    return {
        ...
        "notifications": build_notifications(employees, events, hr_profiles, absences),
        "groups": build_groups(employees, events, hr_profiles, absences),
    }
